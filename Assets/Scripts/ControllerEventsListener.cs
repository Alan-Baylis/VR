...
namespace VRTK.Examples
{
    ...
    public class ControllerEventsListener : MonoBehaviour
    {
        ...
        private GameObject appMenu = null;

        private void Start()
        {
            if (GetComponent<VRTK_ControllerEvents>() == null)
            {
                Debug.LogError("VRTK_ControllerEvents_Listener is required to be attached to a Controller that has the VRTK_ControllerEvents script attached to it");
                return;
            }

            //Setup controller event listeners, saving some resourcing by not initializing the unneeded ones.
        ...
        */
        ...
        private void DoApplicationMenuPressed(object sender, ControllerInteractionEventArgs e)
        {
            ...
            if (appMenu)
            {
                ...
                appMenu = (GameObject)Instantiate(Menu, NewPosition, rotation);
                appMenu.transform.Rotate(0, -180, 0);
